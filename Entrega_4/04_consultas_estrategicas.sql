...
-- CONSULTA 02: CRESCIMENTO DE ACIDENTES GRAVES ANO A ANO
WITH BASE AS (
    SELECT 
        ANO,
...
SELECT
    METEOROLOGIA,
    VISIBILIDADE,
    SUM(VITIMAS_GRAVES) AS TOTAL_ACIDENTES_GRAVES,
    SUM(TOTAL_ACIDENTES) AS TOTAL_ACIDENTES,
    RANK() OVER (ORDER BY SUM(VITIMAS_GRAVES) DESC) AS RANKING_GRAVES
...
GROUP BY TAC_CLASS_ACID, TAC_TIPO_ACID, ACD_ANO, ACD_MES
ORDER BY TAC_CLASS_ACID, TAC_TIPO_ACID, ACD_ANO, ACD_MES;
